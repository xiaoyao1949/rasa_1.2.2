...
class DialogueStateTracker(object):
    ...
    # 这个是rasa core的核心类之一，一个DST类的实例用来保存一个会话的所有信息
    @classmethod
    def from_dict(
            cls,
            sender_id: Text,
        ...
        evts = events.deserialise_events(events_as_dict)  # 事件反序列化
        return cls.from_events(sender_id, evts, slots, max_event_history)  # 事件重放生成tracker
    ...
    def from_events(
        ...
        tracker = cls(sender_id, slots, max_event_history)  # 生成一个空壳
        for e in evts:
            tracker.update(e)  # 逐条重放
        return tracker
    ...
    def __init__(self, sender_id, slots, max_event_history=None):
        ...
        # 很多事件可能存储在了外部（比如数据库中），需要获取到这些事件，重放，然后才能得到会话当前状态。

        # maximum number of events to store
        self._max_event_history = max_event_history
        # list of previously seen events
        # deque 类型。是一个定长的双端队列，可以实现两端append，不利于随机访问。
        ...
        # 明确确认的下一条需要被执行的动作名称 'str'， 初始默认是听，也可以修改为问候一类的
    ...
    def current_state(
        ...
        if event_verbosity == EventVerbosity.ALL:
            evts = [e.as_dict() for e in self.events]  # 所有事件
        elif event_verbosity == EventVerbosity.AFTER_RESTART:
            evts = [e.as_dict() for e in self.events_after_latest_restart()]  # 最近一次重启后发生的事件
        elif event_verbosity == EventVerbosity.APPLIED:
            evts = [e.as_dict() for e in self.applied_events()]  # 最近一次重启后发生的事件，且撤销掉用户用户说撤销的事件，以及应该被撤销的
        else:
            evts = None
    ...
    def events_after_latest_restart(self) -> List[Event]:
        ...
        return list(self.events)[self.idx_after_latest_restart():] # 截至上次重启发生的事件
    ...
    def _reset(self) -> None:
        ...
        # 不删除事件的情况下，把tracker恢复到初始状态
        self._reset_slots()  # 槽值恢复
        self._paused = False
        self.latest_action_name = None
