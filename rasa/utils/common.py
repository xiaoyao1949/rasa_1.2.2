...
def class_from_module_path(
    ...
    # 给一个模块名（和可选的查找路径），返回一个类对象
    import importlib

    # load the module, will raise ImportError if module cannot be loaded
    if "." in module_path:
        # module_path是【模块名.类名】的形式，先拆分出类目和模块目录
        module_name, _, class_name = module_path.rpartition(".")
        # 导入模块
        m = importlib.import_module(module_name)
        # get the class, will raise AttributeError if class cannot be found
        return getattr(m, class_name)  # 从已经导入的模块中返回相应的类对象
    else:
        # module_path直接就是类名。从全局变量里找名为module_path的类，没有的话从局部变量中找
        module = globals().get(module_path, locals().get(module_path))
        if module is not None:
            # 找到了，返回之
            return module
        # 全局和局部都没这个对象，从lookup_path里边找
        if lookup_path:
            # last resort: try to import the class from the lookup path
            m = importlib.import_module(lookup_path)  # 导入模块
            return getattr(m, module_path)  # 从已经导入的模块中返回相应的类对象
        else:
            # 全局、局部、lookup_path 都没有这个类
